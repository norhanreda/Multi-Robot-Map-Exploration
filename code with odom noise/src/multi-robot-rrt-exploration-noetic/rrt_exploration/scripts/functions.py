...
class robot:
    ...
    def odom_callback(self, data):
        ...
        if self.first_run == True:
            ...
            self.previous_z = trans[2]
            self.previous_w=rot[0]
            q = [ trans[0],trans[1],trans[2],rot[0]]
          
            (r,p, theta1) = tf.transformations.euler_from_quaternion(q)
            self.previous_theta1=theta1 
        # # Inject odometry noise
        # noise_std_dev = 20 # You can adjust this value based on your requirements
        # x_noise = np.random.normal(0, noise_std_dev)
        # y_noise = np.random.normal(0, noise_std_dev) 
        a1 = 0.1
        a2 = 0.2
        a3 = 0.0
        a4 = 0.0
        ...
        sd_rot1 = a1*abs(rot1) + a2*d_increment
        sd_rot2 = a1*abs(rot2) + a2*d_increment
        sd_trans = a3*d_increment + a4*(abs(rot1) + abs(rot2))
        
        d_increment +=  np.random.normal(0,sd_trans*sd_trans)
        ...
        self.previous_x = x+d_increment*cos(theta1+rot1)
        self.previous_y = y+d_increment*sin(theta1+rot1)
